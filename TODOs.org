...
* TODO comments scattered away when buffer changed not by user's input but but undo-tree or git pull
* TODO use notify to listen to change in evc file
we only persist to project .evc when the last buffer closed

must have a function to replace active data with data loaded from store

get all buffer belongs to the project and run the init again in a
with-current-buffer this way alleviates the doubled pubsub between buffers and
store
